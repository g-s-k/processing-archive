...
float stroke_size = 0.50; // width of the line
...
boolean h_step_sin = true; // linear (false) or sinusoidal (true)
float h_step_ratio = 5; // -6..-1 and 1..6

boolean ch_v_byte = true; // interpret channel value as byte

color bckg_color = color(15); // background color
...
void keyPressed() {
  // SPACE to save
  if(keyCode == 32) {
    String fn = "frames/res_" + sessionid + hex((int)random(0xffff),4)+"_"+filename+fileext;
    buffer.save(fn);
